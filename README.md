...
- **Présences** (pointage automatique) : l'**heure de début** de la journée est la première activité de l'employé dans son espace de travail (« Mon espace ») et l'**heure de fin** sa dernière activité — rafraîchie par un battement automatique toutes les 5 minutes (et à chaque fois que l'onglet redevient visible). Côté RH : grille mensuelle par employé (début, fin, durée, badge *en cours* pour le jour en cours), **corrections** (date/heures, signalées « corrigé »), **export CSV** du mois (compatible Excel). Côté employé : carte « Ma présence » avec journée du jour et 14 derniers jours.
- **Projets** : projets de l'organisation (client/bailleur, échéance, statuts planifié/en cours/clôturé/annulé) avec **équipe** (membres choisis dans l'équipe) et compteur de tâches ouvertes. Supprimer un projet conserve ses tâches (sans projet).
- **Tâches** (style Asana) : tableau kanban **À faire / En cours / Terminée** — intitulé, description, projet rattaché, employé assigné, **priorité** (basse → urgente), **échéance** (alerte en retard), filtres par projet et par employé. Chaque tâche a un **historique d'échanges** (commentaires tracés avec auteur et date) et une **fiche PDF** imprimable (en-tête de l'organisation, détails, historique, zones de signature) téléchargeable par l'admin **ou par l'employé concerné**.
- **Mon espace (auto-service)** : chaque employé dont l'email correspond à un dossier voit *Admin → Mon espace* — consultation de son solde, **sa présence** (journée en cours, début/fin, temps passé, 14 derniers jours — voir Présences), demande de congé (contrôlée par le solde), retrait de ses demandes en attente, **ses tâches** (commencer, terminer, signaler un avancement, télécharger sa fiche PDF) et la **messagerie RH**, plus les **annonces internes** de l'équipe (épinglées en tête).
- **Messagerie** : échange direct entre chaque employé et les ressources humaines — fils de discussion par employé côté admin (compteur de non-lus, rafraîchissement automatique), bulles de messages côté employé.
- **Documents administratifs** : dépôt de documents de l'organisation (statuts, contrats, décisions, rapports, certificats…) avec **catégorie**, **date d'expiration** (alerte orange ≤ 30 jours, rouge expiré), téléchargement et suppression — stockés hors du site public, accessibles uniquement aux rôles RH.
