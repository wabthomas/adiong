...
export const api = {
  ...
  chat: {
    ...
    dm: (userId) => req('/api/chat/dm', { method: 'POST', body: { user_id: userId }, auth: true }),
    ...
    setMemberRole: (id, userId, role) => req(`/api/chat/conversations/${id}/members/${userId}`, { method: 'PUT', body: { role }, auth: true }),
    removeMember: (id, userId) => req(`/api/chat/conversations/${id}/members/${userId}`, { method: 'DELETE', auth: true }),
    messages: (id) => req(`/api/chat/${id}`, { auth: true }),
    typing: (id) => req(`/api/chat/${id}/typing`, { method: 'POST', body: {}, auth: true }),
    mute: (id, muted) => req(`/api/chat/conversations/${id}/mute`, { method: 'PUT', body: { muted }, auth: true }),
    react: (messageId, emoji) => req(`/api/chat/messages/${messageId}/react`, { method: 'PUT', body: { emoji }, auth: true }),
